...
describe('PlanService', () => {
  ...
  it('permite criar propriedade abaixo do limite no plano Basic', async () => {
    await expect(service.assertPropertyLimit(ctx('basic'), 29)).resolves.toBeUndefined();
  });

  it('bloqueia criação ao atingir 30 propriedades no plano Basic', async () => {
    await expect(service.assertPropertyLimit(ctx('basic'), 30)).rejects.toThrow(
      /Plano Básico permite até 30/,
    );
  });

  it('não impõe limite de propriedades no plano Plus', async () => {
    await expect(service.assertPropertyLimit(ctx('plus'), 10_000)).resolves.toBeUndefined();
  });
