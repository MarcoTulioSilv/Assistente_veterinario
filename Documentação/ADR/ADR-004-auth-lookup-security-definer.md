...
O `AuthService` (Sprint 2, `IAuthService.login(email, password, totpCode?)`) recebe apenas um e-mail — descobrir a qual tenant esse usuário pertence **é o próprio propósito da consulta**. Mas a tabela `users` tem RLS ativo (migration `20260728122247_enable_rls`), e a conexão de runtime (`quironequine_app`) não ignora RLS por desenho (ADR-003 Decisão 1). Qualquer consulta a `users` sem `app.current_tenant` definido retorna zero linhas — não há como chegar ao `tenantId` por esse caminho.
...
Duas funções PostgreSQL `SECURITY DEFINER`, somente leitura, de propriedade da role de migration (`quironequine`, superusuário — por isso a função ignora RLS independente de quem a chama):
...
Novo teste de integração `apps/ms-identity/tests/auth-lookup.test.ts` (mesmo padrão de duas conexões do `rls-isolation.test.ts`), rodando as funções como `quironequine_app` (não como admin):
- `auth_lookup_by_email` acha usuário existente, retorna vazio para e-mail inexistente e para usuário soft-deletado.
- `SELECT * FROM users` direto continua bloqueado para `quironequine_app` sem tenant definido — prova que a nova função não afrouxou o grant geral.
